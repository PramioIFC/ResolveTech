...
class Workflow(unittest.TestCase):
 def test_complete_workflow_and_permissions(self):
  ...
  d=dev.request('demands/'+id)
  self.assertEqual(d['answers']['module'],'Financeiro');self.assertTrue(d['report']['summary']);self.assertTrue(d['report']['suggestedNextSteps'])
  d=dev.request('demands/'+id+'/claim',{'revision':d['revision']});self.assertEqual(d['owner_id'],'developer')
  ...
  self.assertFalse(any(item['id']==id for item in dev.request('state')['demands']))
