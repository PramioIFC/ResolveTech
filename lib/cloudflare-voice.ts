...
export class VoiceCall {
 private pc?:RTCPeerConnection;private stream?:MediaStream;private remote=new MediaStream();private listeners=new Set<Listener>();private poll?:ReturnType<typeof setInterval>;private heartbeat?:ReturnType<typeof setInterval>;private pulled=new Set<string>();private generation=0;
 ...
 async join(audio:HTMLAudioElement){
  if(this.pc)return;const generation=++this.generation;this.update({state:'joining',error:undefined,incoming:false});
  try{
   ...
   const pc=new RTCPeerConnection({iceServers:[{urls:'stun:stun.cloudflare.com:3478'}],bundlePolicy:'max-bundle'});this.pc=pc;
   audio.srcObject=this.remote;audio.autoplay=true;
   pc.ontrack=e=>{this.remote.addTrack(e.track);audio.play().catch(()=>{});this.update({state:'connected'})};
   pc.onconnectionstatechange=()=>{if(pc.connectionState==='failed')this.update({state:'error',error:'A conexão de áudio foi interrompida.'})};
   const track=this.stream.getAudioTracks()[0];const transceiver=pc.addTransceiver(track,{direction:'sendonly'});
   const offer=await pc.createOffer();await pc.setLocalDescription(offer);
   const pushed=await voiceApi(this.demandId,'publish',{sdp:offer.sdp,mid:transceiver.mid,trackName:track.id});
   if(generation!==this.generation||pc.signalingState==='closed')return;await pc.setRemoteDescription(pushed.sessionDescription);
 ...
 private async pull(peers:any[]){
  const pc=this.pc;if(!pc||pc.signalingState==='closed')return;
  for(const peer of peers){
   ...
   try{
    ...
    if(result.requiresImmediateRenegotiation){if(pc.signalingState==='closed')return;await pc.setRemoteDescription(result.sessionDescription);const answer=await pc.createAnswer();await pc.setLocalDescription(answer);await voiceApi(this.demandId,'renegotiate',{sdp:answer.sdp})}
